"""
Original Repository: https://github.com/alexlee-gk/lpips-tensorflow/blob/master/lpips_tf.py
We've upgraded it to tf v2
"""
...
def wrap_frozen_graph(graph_def, inputs, outputs=None):
  def _imports_graph_def():
    tf.compat.v1.import_graph_def(graph_def, name="")
  wrapped_import = tf.compat.v1.wrap_function(_imports_graph_def, [])
  import_graph = wrapped_import.graph
  if outputs == None:
    outputs = import_graph.get_operations()[-1].outputs[0].name
...
def get_lpips_fn(model='net-lin', net='alex', version=0.1):
    ...
    loaded = graph_def.ParseFromString(open(os.path.join(cache_dir, pb_fname), 'rb').read())

    lpips_func = wrap_frozen_graph(
        graph_def, inputs=['0:0', '1:0'])


    def _lpips(input0,input1):
        ...
        distance = lpips_func(input0, input1)
